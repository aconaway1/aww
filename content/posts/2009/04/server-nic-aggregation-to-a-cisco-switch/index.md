...
Let's look at mode 4 a little closer to see what we need to do.  The scenario is that you have _eth0_ plugged into F0/15 of a 2950 and _eth1_ is in F0/16.  You've seen the [configuration for channelling between switches](http://aconaway.com/2008/04/18/getting-started-with-etherchannel/ "AConaway.com -- Getting Started with EtherChannel") before, so you know the basics.  Put the ports in the same _channel-group_ and configure the proper _Port-channel_ interface to do the work.  In this case, we're just configuring the ports to house a host instead of being trunks.
