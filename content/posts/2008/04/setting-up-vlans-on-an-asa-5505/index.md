...
The interface VLANs are what handles the IP addresses. We'll put specific ports in the VLANs in a minute, so hold tight. The configuration is pretty self-explanatory with VLAN11 being the outside (I'net), VLAN 12 being the DMZ (guests), and VLAN13 being the inside (users).  There is, however, the one line that reads _no forward interface Vlan13_. This is the line  that makes dictates which VLAN is a DMZ with respect to the base license and that this VLAN (VLAN12) can't initiate connections to VLAN13. You also might have noticed the line _ip address dhcp setroute_. You can read [one of the earlier articles](/posts/2008/03/default-route-via-dhcp-on-an-asa-5505/ "AConaway.com -- Default Route via DHCP on an ASA 5505") on that guy.
